...
export default function TransactionForm({ accounts, exit }) {
  ...
  const onSubmit = (e) => {
    e.preventDefault()
    console.log(data);
    Inertia.post(route('Inflow:store'), data);
  ...
  return (
    ...
    <div className="card w-full max-w-sm drop-shadow bg-slate-100 border-1 border-black z-100">
        <form className="card-body" onSubmit={onSubmit}>
          <h1 className="text-black text-2xl font-bold text-center">Add Inflow Transaction</h1>
          {errors.account_id && (
                <div className="alert alert-error">{errors.account_id}</div>
          )}
          <div className="form-control">
            <label className="label">Select Account:</label>
            <select className="select select-ghost w-full max-w-xs" onChange={(e) => setData({...data, account_id: e.target.value})}>
          ...
          {errors.date && (
                <div className="alert alert-error">{errors.date}</div>
          )}
          <div className="form-control">
